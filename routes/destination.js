  ...
  try {
    ...
    if (req.query.category) {
      let fetchDestinations = await pool
        ...
        .query(
          "select id, [name], address, description, rating, rateNum, latitude, longitude from Destination where category = @category order by rating desc"
        );
      ...
      let fetchDestinations = await pool
        .request()
        .query(
          "select id, [name], address, description, rating, rateNum, latitude, longitude from Destination order by rating desc"
        );
      res.send(fetchDestinations.recordset);
    }
  } catch (err) {
